			...
			public boolean dispatchKeyEvent(KeyEvent e) {
				switch (e.getID()) {	// Switch for if it was pressed or released
					case KeyEvent.KEY_PRESSED:	// This sets up the entry in the arrays for each key if it is pressed and not already in the array, if it is then it updates the state
						if (Keys.contains(e.getKeyCode())) {	// If it is set up
							SetKey(e.getKeyCode(), true);		// Updates current state
						} else {	// Sets up entry
						...
						break;
					case KeyEvent.KEY_RELEASED:	// If the key was released, this method SHOULD already captures that it was pressed, so it is always set up, 
						if (Keys.contains(e.getKeyCode())) {	//but just in case of lag or other issues, check
							SetKey(e.getKeyCode(), false);		// Updates current state
						} else {	// Sets up entry
							Keys.add(e.getKeyCode());
							KeyPressed.add(false);
							KeyLastState.add(true);
						}
						break;
				}
				return true;
